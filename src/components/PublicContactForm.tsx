...
export const PublicContactForm: React.FC<PublicContactFormProps> = ({ officeForm, officeSlug }) => {
  ...
  // Pre-fill area if valid query param area is provided
  useEffect(() => {
    const areaParam = searchParams.get('area');
    if (areaParam && officeForm.areas.includes(areaParam as any)) {
      setFormData(prev => ({ ...prev, area: areaParam }));
    }
  }, [officeForm, searchParams]);
  ...
  if (success) {
    return (
      <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-200 text-center space-y-6">
  ...
  return (
    <div className="bg-white shadow-sm border border-slate-200 rounded-2xl overflow-hidden" id="contact-form">
      ...
      <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6">
        ...
        <div>
          ...
          <Input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Seu nome completo" />
        </div>
        ...
        <div>
          ...
          <Select required value={formData.area} onChange={e => setFormData({...formData, area: e.target.value})}>
            <option value="">Selecione a área de sua dúvida</option>
