...
Deno.serve(async request => {
  ...
  const service = Deno.env.get('BACKEND_SERVICE_ROLE_KEY') || Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') || Deno.env.get('SUPABASE_SECRET_KEY');
  if (!authorization || !url || !anon || !service) return json({ error: 'Configuração incompleta.' }, 500);
  const userClient = createClient(url, anon, { global: { headers: { Authorization: authorization } } });
  const { data: auth, error: authError } = await userClient.auth.getUser();
  if (authError || !auth.user) return json({ error: 'Autenticação obrigatória.' }, 401);
  ...
  if (!membership) return json({ error: 'Escritório não encontrado ou sem permissão.' }, 403);
  ...
  if (!connection || connection.status !== 'active' || !connection.calendar_id || !connection.sync_enabled) return json({ error: 'Conecte e selecione uma agenda Google antes de sincronizar.' }, 412);
  ...
  if (runError || !run) return json({ error: 'Não foi possível iniciar a sincronização.' }, 500);
  ...
  if (queueError) {
    ...
    return json({ error: 'Não foi possível enfileirar as pendências.', syncRunId: run.id }, 500);
  }
  ...
  return json({ syncRunId: run.id, status: 'running', queued: Number(queued || 0) }, 202);
});
