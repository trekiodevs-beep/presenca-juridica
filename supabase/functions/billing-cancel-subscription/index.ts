...
Deno.serve(async request => {
  ...
  const { data: cancellation, error: requestError } = await admin.from('billing_cancellation_requests').insert({ office_id: officeId, subscription_id: subscription.id, requested_by: userId, effective_at: effectiveAt, protocol, status: 'REQUESTED' }).select('id').single();
  ...
  const { error: localUpdateError } = await admin.from('billing_subscriptions').update({ status: 'CANCEL_AT_PERIOD_END', cancel_at_period_end: true, canceled_at: canceledAt, cancellation_reason: 'Solicitado pelo cliente', updated_at: canceledAt }).eq('id', subscription.id);
  if (localUpdateError) {
    await admin.from('billing_cancellation_requests').update({ status: 'REVERSED' }).eq('id', cancellation.id);
    return json(request, { error: 'Não foi possível registrar o cancelamento.', code: 'cancellation_save_failed' }, 500);
  }
  if (subscription.provider_subscription_id) {
    try { await asaasRequest(`subscriptions/${encodeURIComponent(String(subscription.provider_subscription_id))}`, { method: 'DELETE' }); }
      ...
      console.error('billing cancellation provider request failed', { officeId, message: error instanceof Error ? error.message : String(error) });
      await admin.from('billing_subscriptions').update({ status: subscription.status, cancel_at_period_end: false, canceled_at: null, cancellation_reason: null, updated_at: new Date().toISOString() }).eq('id', subscription.id);
      await admin.from('billing_cancellation_requests').update({ status: 'REVERSED' }).eq('id', cancellation.id);
      return json(request, { error: 'Não foi possível cancelar a renovação. Tente novamente.', code: 'provider_cancellation_failed' }, 502);
    }
  }
  await admin.from('billing_cancellation_requests').update({ status: 'SCHEDULED' }).eq('id', cancellation.id);
  await admin.from('offices').update({ billing_cancel_at_period_end: true, updated_at: canceledAt }).eq('id', officeId);
