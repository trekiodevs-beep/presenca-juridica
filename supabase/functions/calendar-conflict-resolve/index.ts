...
Deno.serve(async request => {
  ...
  if (!auth.user) return json({ error: 'Autenticação obrigatória.' }, 401);
  const body = await request.json().catch(() => ({})); const eventId = String(body.eventId || ''); const decision = String(body.decision || '');
  if (!eventId || !['crm', 'google', 'manual'].includes(decision)) return json({ error: 'Evento e decisão válidos são obrigatórios.' }, 400);
  const { data: event, error: eventError } = await client.from('calendar_events').select('*').eq('id', eventId).maybeSingle(); if (eventError || !event) return json({ error: 'Conflito não encontrado.' }, 404);
  const updates = decision === 'google' ? { sync_status: 'pending', origin: 'google' } : { sync_status: 'pending' };
  const { error } = await client.from('calendar_events').update(updates).eq('id', eventId); if (error) return json({ error: error.message }, 400);
  ...
  return json({ eventId, decision, status: 'pending' });
});
