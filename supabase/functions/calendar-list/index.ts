...
Deno.serve(async request => {
  ...
  if (!authorization || !url || !anon || !secret || !encryptionKey) return json({ error: 'Configuração incompleta.' }, 500);
  ...
  if (error || !connection || connection.status !== 'active') return json({ error: 'Agenda Google não conectada.' }, 412);
  ...
  if (!tokenResponse.ok || !tokens.access_token) return json({ error: 'Não foi possível renovar o acesso Google.' }, 502);
  ...
  if (!calendarsResponse.ok) return json({ error: 'Não foi possível listar as agendas Google.' }, 502);
  return json({ calendars: (calendars.items || []).map((item: Record<string, unknown>) => ({ id: String(item.id), summary: String(item.summary || item.id), description: item.description ? String(item.description) : null, primary: Boolean(item.primary), accessRole: String(item.accessRole || '') })) });
});
