...
Deno.serve(async (request) => {
  ...
  if (!authorization) return Response.json({ error: 'Autenticação obrigatória.' }, { status: 401 });
  const missing = [
  ...
  if (missing.length > 0) {
    ...
    return Response.json({ error: 'Configuração OAuth incompleta.', missing }, { status: 500 });
  }
  ...
  if (!profile?.office_id) return Response.json({ error: 'Usuário sem escritório.' }, { status: 412 });
  ...
  if (error) return Response.json({ error: 'Não foi possível iniciar a autorização.' }, { status: 500 });
  ...
  return Response.json({ authorizationUrl: `https://accounts.google.com/o/oauth2/v2/auth?${params}` });
});
