...
Deno.serve(async request => {
  ...
  if (!['PIX', 'BOLETO', 'CREDIT_CARD'].includes(billingType)) return json(request, { error: 'Forma de pagamento inválida.', code: 'invalid_billing_type' }, 400);
  ...
  try {
    const provider = await asaasRequest(`subscriptions/${encodeURIComponent(String(subscription.provider_subscription_id))}`, { method: 'PUT', body: JSON.stringify({ billingType }) });
